...
class TestUserGet(BaseCase):
    ...
    def test_other_user(self):
        ...
        response3 = requests.get(f"https://playground.learnqa.ru/api/user/1", params=data,
                                   headers={"x-csrf-token": token},
                                   cookies={"auth_sid": auth_sid}
                                   )
        Assertions.assert_json(response3, "username")
        Assertions.assert_json_no_key(response3, "email")
        Assertions.assert_json_no_key(response3, "firstName")
        Assertions.assert_json_no_key(response3, "lastName")
